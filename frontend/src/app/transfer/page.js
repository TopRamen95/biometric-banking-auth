...
export default function Transfer() {
  ...
  useEffect(() => {
    ...
    gsap.from(".transfer-title", { opacity: 0, y: 50, duration: 1 });
    gsap.from(".transfer-form", { opacity: 0, scale: 0.8, duration: 1, delay: 0.5 });
  }, []);
  ...
  const handleVoiceCommand = () => {
    if (transcript.toLowerCase().includes("dashboard")) {
      ...
      router.push("/login");
    } else if (transcript.toLowerCase().includes("transfer")) {
      const match = transcript.match(/\d+/);
      if (match) setAmount(parseFloat(match[0]));
    } else if (transcript.toLowerCase().includes("to")) {
      const recipientMatch = transcript.match(/to\s(\w+)/);
      if (recipientMatch) setRecipient(recipientMatch[1]);
    } else {
  ...
  return (
    <div className="relative w-full min-h-screen bg-gradient-to-br from-[#D74E26] via-[#ff7043] to-[#B6411A] text-white">
      ...
      <div className="ml-64 p-10">
        <div className="transfer-title text-4xl font-bold mb-10">Transfer Funds</div>

        {/* Transfer Form */}
        <div className="transfer-form bg-white/20 p-6 rounded-xl shadow-md backdrop-blur-md">
          ...
          />
          <h3 className="text-xl font-bold mt-6">Amount</h3>
          <input
            type="number"
            value={amount}
