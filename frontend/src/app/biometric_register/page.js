  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setMessage(`Registration failed: ${responseData.error || "Check console for details"}`);
      }
    } catch (error) {
  ...
  return (
    <div className="relative w-full h-screen bg-gradient-to-br from-[#D74E26] via-[#ff7043] to-[#B6411A] flex flex-col items-center justify-center text-white overflow-hidden">
      ...
      <div className="max-w-md w-full bg-white/20 p-8 rounded-lg shadow-lg backdrop-blur-md mt-32 overflow-auto">
        <h1 className="text-3xl font-bold text-center mb-6 text-white drop-shadow-lg">Biometric Register</h1>
        ...
        <form ref={formRef} onSubmit={handleSubmit} className="space-y-6">
          ...
          {voiceData && (
            <p className="text-white text-center mb-4">Voice recording ready: {voiceData.name}</p>
          )}
          ...
          <div className="flex justify-between mb-4">
            <button
              ...
              className="px-6 py-2 rounded-md bg-white text-[#D74E26] font-semibold shadow-lg hover:bg-blue-800 hover:text-white transition"
            >
              Capture Face
            </button>
            ...
            <button
              ...
              className={`px-6 py-2 rounded-md bg-white text-[#D74E26] font-semibold shadow-lg hover:bg-blue-800 hover:text-white transition ${recording ? "bg-red-500" : ""}`}
            >
              {recording ? "Stop Recording" : "Start Recording"}
            </button>
          </div>

          <button type="submit"className="cta-btn w-full px-6 py-3 bg-white text-[#D74E26] font-semibold rounded-lg shadow-lg hover:bg-blue-800 hover:text-white transition">
            Register
          </button>
