...
class CustomUserAdmin(UserAdmin):
    list_display = ('username', 'phone_no', 'email', 'is_admin', 'is_customer','is_service_agent', 'is_active')

    fieldsets = (
        ('Personal Info', {'fields': ('username', 'phone_no', 'email', 'password')}),
        ('Permissions', {'fields': ('is_active', 'is_staff', 'is_superuser', 'is_customer', 'is_admin', 'is_service_agent')}),  # ✅ Fix here
        ('Important Dates', {'fields': ('last_login', 'date_joined')}),
    )

    add_fieldsets = (
        ('Personal Info', {'fields': ('username', 'phone_no', 'email', 'password1', 'password2')}),
        ('Permissions', {'fields': ('is_active', 'is_staff', 'is_superuser', 'is_admin', 'is_service_agent', 'is_customer')}),  # ✅ Fix here
    )
